...
public:
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    while (iter != entry->end()) {
        ...
        // CuLifter's special registers have become loads. They must execute
        // inside the lane loop; hoisting all loads freezes every lane at zero.
        if (isa<AllocaInst>(I)) {
            ++iter;
            continue;
        }
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    while (!conditionalBarriers.empty()) {
      ...
      while (!if_body.empty()) {
        ...
        if_body.pop();
        // The conditional body ends at its postdominating merge. Walking
        // beyond it manufactures boundaries in unrelated later lane branches
        // (including lane-zero stores after a reduction).
        if (curr == merge_point)
          continue;
...
public:
  ...
  bool runOnLoop(Loop *L, LPPassManager &LPM) {
    ...
    if (!is_conditional_loop)
      return 0;
    // A loop containing both warp and CTA phases needs CTA boundaries.
    // Otherwise the outer collapse walks across its backedge, finds multiple
    // preceding barriers, and silently leaves the prologue/loop unwrapped.
    is_warp = is_warp && !has_block_scope;
