  ...
  void analyze(Function &F) {
    ...
    for (Instruction &I : instructions(F)) {
      ...
      if (isDivergenceSource(&I)) {
        ...
        Worklist.push(&I);
        // Printing an instruction rebuilds LLVM's slot tracker for the whole
        // function. Doing it for every divergence source made large tensor
        // kernels spend minutes formatting diagnostics in normal builds.
        if (cupbop_debug()) {
    ...
    if (cupbop_debug()) fprintf(stderr, "[div] total divergent values: %zu\n", DivergentValues.size());
  }
...
bool ShouldNotBeContextSaved(llvm::Instruction *instr,
  ...
  if (instr->getMetadata("cpu.block.shared")) return true;
  if (cupbop_cpu_per_block_context() && !instr->getMetadata("cpu.lane.value")) return true;
  ...
  if (load != NULL) {
    ...
    // Source coordinate loads are rematerialized at every use by the CPU
    // adapter. Loads inserted by collapse are loop control, not lane state.
    if (load_addr == M->getGlobalVariable("intra_warp_index") ||
        load_addr == M->getGlobalVariable("inter_warp_index"))
      return true;
  ...
  // LLVM 18
  // CPU serialization cannot rely on the upstream incomplete memory/control
  // divergence analysis to share live state. Conservatively preserve every
  // cross-region source value; later proven-uniform hoisting is optional.
  return !cupbop_cpu_per_block_context() && !DI.isDivergent(instr);
  //return false;
}
  ...
  for (auto bb = F->begin(); bb != F->end(); bb++) {
    for (auto ii = bb->begin(); ii != bb->end(); ii++) {
      if (llvm::AllocaInst *i = dyn_cast<AllocaInst>(ii)) {
        if (i->getMetadata("cpu.block.shared")) continue;
...
void handle_local_variable_intra_warp(std::vector<ParallelRegion> &PRs,
    ...
    for (auto bb = F->begin(); bb != F->end(); bb++) {
      for (auto ii = bb->begin(); ii != bb->end(); ii++) {
        if (isa<AllocaInst>(&(*ii))) {
          ...
          if (alloc->getMetadata("cpu.block.shared")) continue;
          ...
          // replicate
          // added: Nov 20 2023
          bool allStoreNonDivergence = !cupbop_cpu_per_block_context();
  ...
  // Convert such allocas to per-thread _intra_warp_ arrays.
  {
    ...
    for (auto &F_inst : F->getEntryBlock()) {
      ...
      if (!AI) continue;
      if (AI->getMetadata("cpu.block.shared")) continue;
      // Skip context arrays and shfl emulation temporaries
      ...
      if (needs_check) {
        ...
        for (auto *U : AI->users()) {
          if (auto *SI = dyn_cast<StoreInst>(U)) {
            found_store = true;
            bool div = cupbop_cpu_per_block_context() || DI.isDivergent(SI->getValueOperand());
...
BasicBlock *insert_loop_cond(llvm::BasicBlock *InsertCondBefore,
  ...
  if (!intra_warp_loop) {
    ...
    if(cupbop_cpu_per_block_context() && need_nested_loop){
      auto* F=InsertCondBefore->getParent();unsigned size=0;
      if(F->hasFnAttribute("cpu.block_size"))F->getFnAttribute("cpu.block_size").getValueAsString().getAsInteger(10,size);
      if(size%32){
        auto* warp=createLoad(builder,M->getGlobalVariable("inter_warp_index"));
        auto* lane=createLoad(builder,intra_warp_index);
        auto* logical=builder.CreateAdd(builder.CreateMul(warp,builder.getInt32(32)),lane);
        cmpResult=builder.CreateAnd(cmpResult,builder.CreateICmpULT(logical,builder.getInt32(size)));
      }
    }
  }
...
extern void normalize_cpu_barrier_joins(llvm::Function&);
extern void preserve_cpu_barrier_free_branches(llvm::Function&);
extern void normalize_cpu_barrier_forks(llvm::Function&);
...
//                "[WARNING] match single conditional branch with HARD CODE\n");
      ...
      // A branch-only block in lifted IR is ordinary lane control flow, not
      // a synchronization boundary. This also holds when the outer pass sees
      // the conditional branches introduced for inner CPU lane loops.
      if (cupbop_cpu_per_block_context())
        is_single_conditional_branch_block = false;
    ...
    // Inspect the entire region, including its final block and every arm.
    // The upstream walk stopped on reaching `exit` without inspecting it:
    // branch-only entry -> arithmetic/store exit was silently discarded.
    for(auto* BB:current_region.wrapped_block)
      for(auto& I:*BB)
        if(!isa<BranchInst>(I)||cast<BranchInst>(I).isConditional())
          is_useless=false;
    auto iter = entry;
    bool any_divergent_var = cupbop_cpu_per_block_context();
    if (!cupbop_cpu_per_block_context()) {
  ...
  virtual bool runOnFunction(Function &F) {
    ...
    // Removing warp boundaries changes which CTA boundary precedes a join.
    // Normalize again at this hierarchy level, after the lane collapse.
    if(cupbop_cpu_per_block_context() && !intra_warp_loop) {
      // Inner collapse removes warp collectives. Diamonds that now contain
      // no CTA collective belong to one outer warp loop; retaining their
      // synthetic cuts yields overlapping loops sharing inter_warp_index.
      preserve_cpu_barrier_free_branches(F);
      normalize_cpu_barrier_forks(F);
      normalize_cpu_barrier_joins(F);
    }
    ...
    DT->recalculate(F);PDT->recalculate(F);LI.releaseMemory();LI.analyze(*DT);
...
void insert_warp_loop(llvm::Module *M) {
  ...
  if (need_loop_serialize) {
    if (need_nested_loop) {
      ...
      bool singleWarp = true;
      for (auto &F : *M) if (isKernelFunction(M, &F)) {
        unsigned size = 0;
        if (!F.hasFnAttribute("cpu.block_size") ||
            F.getFnAttribute("cpu.block_size").getValueAsString().getAsInteger(10, size) || size > 32)
          singleWarp = false;
      }
      // A CPU block with one warp has no inter-warp scheduling dimension.
      if (!singleWarp) Passes.add(new InsertWarpLoopPass(!intra_warp, schedule));
